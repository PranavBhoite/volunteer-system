...
const HelpSection = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    time: '',
    place: '',
    extraVolunteers: false
  });

  useEffect(() => {
    const handleHelpAction = (e) => {
      if (e.detail === 'create') {
        setFormVisible(true);
      } else if (e.detail === 'approved' || e.detail === 'disapproved') {
        setStatusFilter(e.detail);
      } else if (e.detail === 'all') {
        setStatusFilter("");
      }
    };
    window.addEventListener('help-action', handleHelpAction);
    return () => window.removeEventListener('help-action', handleHelpAction);
  }, []);
      ...
      <Row className="align-items-center justify-content-between mb-4">
        <Col>
          <h3 className="events-heading">
            ...
            Events List {statusFilter && `(Filtered: ${statusFilter})`}
          </h3>
        </Col>
        ...
        <Table bordered responsive className="table-bordered-blue">
          ...
          <tbody>
            {events.map((event, idx) => {
              const rowColor = idx % 2 === 0 ? '#FFFFFF' : '#cef6ff';
              return (
                <tr key={event._id}>
                  ...
                  <td style={{
                    ...
                    color: event.status === 'approved' ? '#4CAF50' :
                      event.status === 'disapproved' ? '#F44336' : '#333',
                    fontWeight: 500
                  }}>
                    {event.status}
                  </td>
